...
// I made my own point struct for mousepos because i realized i never need anything bigger than a short for screen dimension stuff like mouse position.
...
void foxxey_Exit(UINT8 exitCode) { // how can i close all threads before exiting?? i need a thread to exit!. Answer: I made an array of all thread handles that will exist, and i will close every single one thats not the current one, and then with this remaining thread, i do exit.
...
DWORD WINAPI windowLogic_function(LPVOID lpThreadParameter) {
    while (1) { // do wnidow stuff
        ...
        if (resizing) { // resizing, so change window size
            ...
            free(bitmap);
            bitmap = malloc((windowWidth +(4 -((windowWidth *3) %4))) *windowHeight *4 *sizeof(byte));
        ...
        for (int row = 0; row <= windowHeight -1; row++) { // test
            for (int col = 0; col <= windowWidth -1; col++) {
                if (row <= 10 -1) {
                    bitmap[(row *((windowWidth *3) +paddingBytesAmount)) +(col *3 +0)] = 0; // B
                    bitmap[(row *((windowWidth *3) +paddingBytesAmount)) +(col *3 +1)] = 255; // G
                    bitmap[(row *((windowWidth *3) +paddingBytesAmount)) +(col *3 +2)] = 255; // R
                } else {
                    bitmap[(row *((windowWidth *3) +paddingBytesAmount)) +(col *3 +0)] = 0; // B
                    bitmap[(row *((windowWidth *3) +paddingBytesAmount)) +(col *3 +1)] = 0; // G
                    bitmap[(row *((windowWidth *3) +paddingBytesAmount)) +(col *3 +2)] = 0; // R
                }
            }
        }
...
EXPORT void startWindow(void) {
    ...
    bitmap = malloc((200 +(4 -((200 *3) %4))) *200 *4 *sizeof(byte)); // its *4 because even though we have 3 colours which each is a byte, win32 api REQUIRES bitmaps be a multiple of 4 bytes in size. so for every pixel, i gotta add that fourth unread value :/
