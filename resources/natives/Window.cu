...
void foxxey_Exit(INT8 exitCode) { // how can i close all threads before exiting?? i need a thread to exit!. Answer: I made an array of all thread handles that will exist, and i will close every single one thats not the current one, and then with this remaining thread, i do exit.
    ...
    free(bitmap); // i malloced this one
...
DWORD WINAPI windowLogic_threadFunction(LPVOID lpThreadParameter) {
    ...
    while (1) { // do wnidow stuff
        ...
        clearWindow_kernel<<<gridDim_, blockDim_>>>(d_bitmap, windowWidth, windowWidthReciprocal, windowHeight, pixelsAmount, paddedRowBytesAmount, backgroundColour); // 1 thread per pixel
        drawHollowCircleWithBorder_kernel<<<gridDim_, blockDim_>>>(d_bitmap, windowWidth, windowWidthReciprocal, windowHeight, pixelsAmount, paddedRowBytesAmount, circleMiddleX, circleMiddleY, radiusSquared, innerRadiusSquared, radius, innerAABBHalfLength, circleColour); // 1 thread per pixel
        // drawHollowCircleWithBorder_kernel<<<gridDim_, blockDim_>>>(d_bitmap, windowWidth, windowWidthReciprocal, windowHeight, pixelsAmount, paddedRowBytesAmount, circleMiddleX, circleMiddleY, radiusSquared, innerRadiusSquared, circleColour); // 1 thread per pixel

        // for (int i = -(windowWidth /2); i <= windowWidth /2; i+=30) { // test
        //     for (int i2 = -(windowHeight /2); i2 <= windowHeight /2; i2+=30) {
        //         drawHollowCircleWithBorder(d_bitmap, windowWidth, windowHeight, circleMiddleX +i, circleMiddleY +i2, radiusSquared, innerRadiusSquared, circleColour);
        //     }
        // }

        cudaMemcpy(bitmap, d_bitmap, bytesAmount, cudaMemcpyDeviceToHost); // to, from
        cudaFree(d_bitmap); // free on device
